...
BEGIN
  ...
  -- 2. RAW DATA CLEANING (All 6 Sources)
  voc_raw AS (
    SELECT DISTINCT
  ...
  -- 3. BASE CONSOLIDATION (Full Outer Join)
  base AS (
    SELECT 
  ...
  -- 7. STAGE 4: AGGREGATE PTG & PRE-SCORES
  stage_4 AS (
    SELECT *,
  ...
  -- 8. STAGE 5: FINAL SCORECARD METRICS
  stage_5 AS (
    SELECT *,
      -- Productivity Points & Final Category
      ...
      -- Accuracy Points & Final Category
  ...
  -- 9. STAGE 6: FINAL CONSOLIDATED SCORE
  final_scores AS (
    SELECT *,
      -- FINAL SCORE: Average of all points (Ignora NULLs automáticamente)
      SAFE_DIVIDE(
  ...
  SELECT 
    ...
    -- [QA, VOC, TD, PPD, PRODUCTIVITY, ACCURACY BLOCKS with Scores...]
    -- (Omitidos en vista pero incluidos en el código final para mantener orden)
    ...
    -- THE FINALE: CONSOLIDATED RESULTS
    final_score,
    CASE 
